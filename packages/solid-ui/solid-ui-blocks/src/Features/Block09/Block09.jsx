...
const FeaturesBlock06 = ({ content: { text, buttons, reverse } }) => {
  ...
  const fetchSingleCaseStudy = async (slug) => {
    try {
      const response = await fetch(`https://emergedigital.ae/wp-json/wp/v2/case-studies?slug=${slug}`);
  ...
  const fetchCaseImage = async (id) => {
    try {
      const response = await fetch(`https://emergedigital.ae/wp-json/wp/v2/media/${id}`);
